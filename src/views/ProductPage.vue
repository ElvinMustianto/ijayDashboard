          ...
          <table class="table table-hover align-middle">
            ...
            <tbody>
              ...
              >
                ...
                <td>
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        v-if="img.isPrimary"
                        class="position-absolute top-0 start-0 bg-success text-white rounded-circle"
                        style="
                          font-size: 8px;
                          width: 12px;
                          height: 12px;
                ...
                <td>
                  <span
                    :class="`badge ${item.isActive ? 'bg-success' : 'bg-warning'}`"
                  >
                ...
                <td>
                  ...
                  <button
                    ...
                    @click="handleDelete(item._id || item.tempId)"
                  >
              ...
              <div class="mb-3">
                ...
                <div class="d-flex flex-wrap gap-2 mb-2">
                  ...
                  >
                    ...
                    <div
                      v-if="img.isPrimary"
                      class="position-absolute top-0 start-0 bg-success text-white rounded-circle d-flex align-items-center justify-content-center"
                    ...
                    <!-- Actions -->
                    <div
                      class="position-absolute bottom-0 start-0 w-100 d-flex justify-content-between px-1"
                    >
                      <button
                        v-if="!img.isPrimary"
                        type="button"
...
import { setPrimaryImage, deleteImage } from "@/services/image.service.js";
...
export default {
  ...
  methods: {
    ...
    showModal(type) {
      ...
      if (type === "detail") {
        if (!this.productDetailModal) {
          this.productDetailModal = new Modal(
    ...
    closeModal(type) {
      if (type === "product") this.productModal?.hide();
      if (type === "detail") this.productDetailModal?.hide();
    },
    ...
    handleImageUpload(e) {
      ...
      const remaining = 5 - this.totalImageCount;
      if (remaining <= 0) {
      ...
      this.newImageFiles.push(...selected);
    ...
    async deleteExistingImage(index) {
      const img = this.editingProduct.images[index];
      ...
      try {
        await deleteImage(img._id); // 🔥 API
        this.editingProduct.images.splice(index, 1); // 🔥 UI
      } catch (err) {
    ...
    async makePrimary(imageId) {
      ...
      try {
        ...
        this.editingProduct.images = this.editingProduct.images.map(
          (img) => ({
            ...img,
    ...
    async saveProduct() {
      ...
      try {
        ...
        fd.append("isActive", String(this.editingProduct.isActive));

        if (this.editingProduct.description)
          fd.append("description", this.editingProduct.description);
        if (this.editingProduct.discountPrice)
          fd.append(
            "discountPrice",
            String(this.editingProduct.discountPrice),
          );
        if (this.editingProduct.stock != null)
          fd.append("stock", String(this.editingProduct.stock));
        ...
        if (this.editingProduct._id) {
          await updateProduct(this.editingProduct._id, fd);
        } else {
