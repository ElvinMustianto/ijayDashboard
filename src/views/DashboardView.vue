...
<template>
  <main class="p-3 p-md-4 flex-grow-1">
    ...
    <div class="row g-4 mb-4">
      <div class="col-12 col-md-6 col-lg-3" v-for="(card, index) in stats" :key="index">
    ...
    <div class="card border-0 shadow-sm mb-4">
      ...
      <div class="card-body p-0">
        ...
        <table v-else class="table table-hover mb-0">
          ...
          <tbody>
            <tr v-for="(company, idx) in companies" :key="company._id || company.tempId">
              <td>{{ idx + 1 }}</td>
              <td>{{ company.name }}</td>
              <td>{{ company.industry }}</td>
              <td>{{ company.email }}</td>
              <td>
                <span :class="company.isActive ? 'badge bg-success' : 'badge bg-warning'">
                  {{ company.isActive ? 'Active' : 'Inactive' }}
                </span>
              </td>
              <td class="text-end">
                <button class="btn btn-sm btn-info me-1" @click="viewCompany(company)">
                  <i class="bi bi-eye"></i>
                </button>
                <button class="btn btn-sm btn-outline-primary me-1" @click="editCompany(company)">
                  <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-outline-danger" @click="handleDeleteCompany(company._id || company.tempId)">
    ...
    <div class="card border-0 shadow-sm mb-4">
      ...
      <div class="card-body p-0">
        ...
        <table v-else class="table table-hover mb-0 align-middle">
          ...
          <tbody>
            <tr v-for="(product, idx) in products.slice(0,5)" :key="product._id || product.tempId">
              <td>{{ idx + 1 }}</td>
              <td>
                <div v-if="product.images && product.images.length > 0" class="d-flex flex-wrap gap-1" style="max-width: 120px">
                  <img
                  ...
                  />
                  <div v-if="product.images.length > 3" class="d-flex align-items-center justify-content-center rounded bg-light text-muted"
              ...
              <td class="text-end">
                <button class="btn btn-sm btn-info me-1" @click="viewProduct(product)">
                  <i class="bi bi-eye"></i>
                </button>
                <button class="btn btn-sm btn-outline-primary me-1" @click="editProduct(product)">
                  <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-outline-danger" @click="handleDeleteProduct(product._id || product.tempId)">
                  <i class="bi bi-trash"></i>
                </button>
              </td>
            </tr>
    ...
    <div class="modal fade" id="companyModal" tabindex="-1" aria-hidden="true" ref="companyModal">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <form @submit.prevent="saveCompany">
            <div class="modal-header">
              <h5 class="modal-title">{{ editingCompany._id ? 'Edit Company' : 'Add Company' }}</h5>
    ...
    <div class="modal fade" id="companyDetailModal" tabindex="-1" aria-hidden="true" ref="companyDetailModal">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ detailCompany.name }}</h5>
            <button type="button" class="btn-close" @click="closeCompanyDetailModal"></button>
          </div>
          <div class="modal-body">
            <div class="row g-3">
              <div class="col-12 col-md-6">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" @click="closeCompanyDetailModal">Close</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal fade" id="productDetailModal" tabindex="-1" aria-hidden="true" ref="productDetailModal">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" @click="closeProductDetailModal">Close</button>
          </div>
        </div>
      </div>
    </div>
...
<script>
import { getCompanies, deleteCompany, createCompany, updateCompany } from '@/services/company.service.js'
...
export default {
  ...
  methods: {
    ...
    viewCompany(company) {
      ...
      if (!this.companyDetailModal) {
        this.companyDetailModal = new Modal(this.$refs.companyDetailModal)
      }
    ...
    editCompany(company) {
      ...
      if (!this.companyModal) {
        this.companyModal = new Modal(this.$refs.companyModal)
      }
      this.companyModal.show()
    },
    closeCompanyModal() { this.companyModal?.hide() },
    closeCompanyDetailModal() { this.companyDetailModal?.hide() },
    async saveCompany() {
      this.savingCompany = true
      try {
        if (this.editingCompany._id) {
          await updateCompany(this.editingCompany._id, this.editingCompany)
        } else {
          await createCompany(this.editingCompany)
        }
    ...
    async handleDeleteCompany(id) {
      if (!confirm('Are you sure want to delete this company?')) return
      try {
    ...
    closeProductModal() { this.productModal?.hide() },
    closeProductDetailModal() { this.productDetailModal?.hide() },
    async saveProduct() {
